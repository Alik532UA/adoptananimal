...
| Пастка                         | Що саме                                                                                                                                                                                                                                                                    |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Порт dev-сервера — **5173**    | `.claude/launch.json` цього проєкту. Це типовий порт Vite, тобто спільний з будь-яким сусіднім проєктом: якщо на ньому вже висить чужий dev-сервер, дивитись будеш чужий сайт. Тести цього не торкаються — Playwright піднімає власний `preview` на 4173 зі `--strictPort` |
| `base` виводиться з оточення   | `svelte.config.js` читає `process.env.BASE_PATH ?? ''`. Не хардкодь шлях                                                                                                                                                                                                   |
| Абсолютні URL — не з `base`    | `SITE_ORIGIN` + `SITE_BASE` із `src/lib/config.ts`. У пререндері SvelteKit переписує `base` з `$app/paths` на **відносний** префікс, і canonical виходить як `https://host/../../images/x.jpg`                                                                             |
...
| Джерело даних — зовнішній сайт | порядок тварин, імена, стать, вік, порода, розмір, колір, опис і статус адопції звіряються зі старим діючим `https://www.adoptananimal.in.ua/`, а не вигадуються                                                                                                           |
